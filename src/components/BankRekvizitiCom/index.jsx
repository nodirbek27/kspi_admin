...
function BankRekvizitiCom() {
  ...
  const fechtData = async () => {
    try {
      const response = await APIBankRekviziti.getBankRekviziti();
      setDatas(response.data);
    } catch (error) {
      console.error("Xatolik yuz berdi!", error);
    }
  };
  // formik for requisite Banke
  const formik = useFormik({
    ...
    onSubmit: async (values, onSubmitProps) => {
      ...
      try {
        // POST
        if (!edit) {
          await APIBankRekviziti.postBankRekviziti(data);
        }
        // PATCH
        else {
          await APIBankRekviziti.patchBankRekviziti(id, data);
          setEdit(false);
          setId(null);
        }
  ...
  const handleDelete = async (id) => {
    try {
      await APIBankRekviziti.delBankRekviziti(id);
      fechtData();
    } catch (error) {
      console.error("Xatolik yuz berdi!", error);
    }
...
export default BankRekvizitiCom;
