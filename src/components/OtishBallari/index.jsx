...
import APIOtishBallari from "../../services/otishBallari";
...
const OtishBallari = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const response = await APIOtishBallari.get();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (editItemId) {
          await APIOtishBallari.put(editItemId, formData);
        } else {
          await APIOtishBallari.post(formData);
        }
        fetchItems();
        resetForm();
        setEditItemId(null);
