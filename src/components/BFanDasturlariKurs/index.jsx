...
function BakalavrFanDasturlariKursCom() {
  ...
  const fechtData = async () => {
    try {
      const response = await APIBFanDasturlariKurs.get();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, onSubmitProps) => {
      ...
      try {
        // POST
        if (!edit) {
          await APIBFanDasturlariKurs.post(data);
        }
        // PATCH
        else {
          await APIBFanDasturlariKurs.patch(id, data);
          setEdit(false);
          setId(null);
        }
  ...
  const handleDelete = async (id) => {
    try {
      await APIBFanDasturlariKurs.del(id);
      fechtData();
    } catch (error) {
      console.error("Xatolik yuz berdi!", error);
    }
...
export default BakalavrFanDasturlariKursCom;
