...
import APITalabaMagistr from "../../services/talabaMagistr";
...
const TalabaMagistr = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const response = await APITalabaMagistr.get();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (editItemId) {
          await APITalabaMagistr.put(editItemId, formData);
        } else {
          await APITalabaMagistr.post(formData);
        }
        fetchItems();
        resetForm();
        setEditItemId(null);
