...
function BakalavrOquvRejaTurCom() {
  ...
  const fechtData = async () => {
    try {
      const response = await APIBOquvRejaTur.get();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, onSubmitProps) => {
      ...
      try {
        // POST
        if (!edit) {
          await APIBOquvRejaTur.post(data);
        }
        // PATCH
        else {
          await APIBOquvRejaTur.patch(id, data);
          setEdit(false);
          setId(null);
        }
...
export default BakalavrOquvRejaTurCom;
